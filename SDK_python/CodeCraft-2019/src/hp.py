...
def get_graph(road_df_):
    graph_pair = []
    not_duplex = []
    node_list = []
    for start, end, isDuplex in zip(road_df_['from'], road_df_['to'], road_df_['isDuplex']):
        if isDuplex:
            graph_pair.append([str(start), str(end)])
            if str(start) not in node_list:
                node_list.append(str(start))
            if str(end) not in node_list:
                node_list.append(str(end))
        else:
            not_duplex.append([str(start), str(end)])

    return graph_pair, node_list
