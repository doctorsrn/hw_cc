...
class Graph: 
    ...
    def direct_edge(self, v1, v2): 
        """ The method searches for the direct edge from v1 to v2. """
        if v1 in self.graph: 
    ...
    def get_subgraph(self, nodes):
        """ The method returns subgraph with selected nodes.
             Creates a new graph object only with selected ones
             vertices and their corresponding edges.
             If the indicated vertex does not exist, then it throws an error. """

        # copying the graph only with selected vertices
        print("self.graph.items():", len(self.graph.items()))
        print("self.graph.items():", (list(self.graph.keys())[1]))
        ke = (list(self.graph.keys())[0])
        print("self.graph[xx]:", (self.graph[ke]))
