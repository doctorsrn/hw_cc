...
class TransportNetwork(graph.Graph): 
    ...
    def load_from_file(filename):
        """ Factory method that creates a transport network from a file."""
        network = TransportNetwork()
        print("Creating a transport network from a file: %s..." % filename)
        
        file = open(filename, "r")
        try: 
            no_of_lines = int(file.readline())
        except ValueError: 
            print("Incorrect number of bus lines!")
            file.close()
            return None
    ...
    @staticmethod
    def load_from_adjacency_list(adj):
        """for undirected connection:adj:{1: {2: 2.0, 7: 2.0}, 2: {8: 2.0, 1: 2.0, 3: 2.0}, ...}"""
        network = TransportNetwork()
        for key, value in adj.items():
            for end, weight in value.items():
                network.add_undirected_connection(0, Stop(str(key)), Stop(str(end)), travel_time=weight)
        
        return network

    def load_from_adjacency_list(adj):
        """for directed connection:adj:{1: {2: 2.0, 7: 2.0}, 2: {8: 2.0, 1: 2.0, 3: 2.0}, ...}"""
        network = TransportNetwork()
        for key, value in adj.items():
            for end, weight in value.items():
                network.add_connection(Connection(0, Stop(str(key)), weight, Stop(str(end))))
        
        return network
    
    def __str__(self): 
        """ Text representation of the transport network."""
        output = "Public transport network: "
        for key in self.graph: 
            output += "%s: " % str(key) + "\n"
            for conn in self.graph[key]: # loop after connections
                output += "%s (line:%d, time:%d), " % (
    ...
    def add_undirected_connection(self, line_no, stop1, stop2, travel_time):
        """ The method adds a connection between stops at a given time
             simultaneously in both directions stop1-> stop2 and stop2-> stop1
             for line_no. """
    ...
    def get_subnetwork(self, stops): 
        """ The method returns to the public transport network,
             containing the indicated stops."""
