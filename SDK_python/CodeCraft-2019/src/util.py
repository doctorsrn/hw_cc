...
def build_adjacency_list(cross_df, road_df):
    ...
    """
    # 邻接表结构： 使用嵌套字典：{节点：{相邻节点1：[边ID，边权重], 相邻节点2：[边ID，边权重], '''}}
    adjacency_list = {}
    weight = 0
    next_cross_id = 0
...
def build_ad_list_without_edge_id(cross_df, road_df):
    """
    brief: 从cross_df, road_df得到邻接表
    :param cross_df:
    :param road_df:
    :return:
    """
    # 邻接表结构： 使用嵌套字典：{节点：{相邻节点1：边权重, 相邻节点2：边权重, '''}}
...
def get_path(adl_list_w, start, end, use_networkx=True):
    ...
    :param start:
    :param end:
    :param use_networkx=True: 默认根据networkx库导入情况决定使用那种方法获取路径
    :return:
    """
    global USE_NETWORKX
...
def get_path1(adl_list_w, start, end):
    path = shortest_path(adl_list_w, start, end)
